...
class Meshes(): 
    ...
    def init_mesh(self):
        ...
        T[int(self.nx/2), int(0.2*self.nx):int(0.8*self.nx+1)] = 100
        T[int(self.nx/2-1), int(0.2*self.nx):int(0.8*self.nx+1)] = 100
        self.T = T.copy()
        self.T0=T.copy()
        
        
        DiffTotalX = np.zeros((self.nx, self.ny)) 
        ...
        DiffTotalY = np.zeros((self.nx, self.ny)) 
